...
function normalizeHistoryRangeMinutes(minutes: number): number {
  return (ALLOWED_HISTORY_RANGE_MINUTES as readonly number[]).includes(minutes)
    ? minutes
    : 1;
}
...
function updateChartReadings(point: HistoryPoint | null, hasTemp: boolean) {
  ...
  if (cpuReading) {
    cpuReading.textContent =
      point != null ? `CPU ${point.cpuPercent.toFixed(1)}%` : "CPU --";
  }
  if (memReading) {
    memReading.textContent =
      point != null ? `内存 ${point.memoryPercent.toFixed(1)}%` : "内存 --";
  }
  if (tempReading) {
...
function drawHistoryChart(points: HistoryPoint[]) {
  ...
  const tempValues = drawPoints
    .map((p) => p.cpuTempCelsius)
  ...
  const yPercent = (v: number): number =>
    padY + plotH * (1 - Math.max(0, Math.min(100, v)) / 100);

  const yTemp = (v: number): number =>
    padY + plotH * (1 - (v - tempMin) / tempRange);
...
function fillSettingsForm(
  ...
  const historyRange = document.querySelector(
    "#setting-history-range",
  ) as HTMLSelectElement | null;
  const preciseTemp = document.querySelector(
    "#setting-precise-temp",
  ) as HTMLInputElement | null;

...
当前版本：`0.1.1` · 标识符：`com.sysmon.tray` · 产品名：`SysMon`
...
- 左键点击：切换主面板显示/隐藏（首次靠近托盘；之后恢复上次拖动位置）
- 右键菜单：显示面板 / 叠加层（可勾选）/ 设置 / 退出
- 主面板 `380×690`：无边框、毛玻璃、置顶、不进任务栏；标题栏可拖动并记住位置
- 失焦自动隐藏（设置页打开时除外）；关闭仅隐藏，进程继续常驻

### 桌面叠加层

- 可选启用，默认关闭；托盘菜单与设置页双向同步
- 收起 `310×38`：CPU / 内存 / 温度 / 上下行网速
- 展开 `310×176`：带进度条的详情视图
...
- 开启通知时会请求权限并发送测试通知；开发态可能显示为 PowerShell，正式安装包更稳定
